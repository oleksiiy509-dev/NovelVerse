...
function providerResolution(provider: unknown) {
  ...
  const requested = explicit || primary || "openai";
  return {
...
export async function resolveDefaultProvider() { return "openai"; }
...
export function getAudioProvider(providerId = env("NOVELVERSE_TTS_PROVIDER", "openai")) { const selected = resolveProviderId(providerId); if (selected === "piper") return providers.get("piper") || new PiperTtsProvider(); return providers.get(selected) || providers.get("unconfigured")!; }
...
export function resolveProviderId(provider: unknown) {
  ...
  return !requested || requested === "default" || requested === "auto" ? "openai" : requested;
}
