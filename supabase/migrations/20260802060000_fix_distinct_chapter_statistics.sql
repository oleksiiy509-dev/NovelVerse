-- Chapter numbers, rather than physical import rows, define a novel's chapter
-- count. Refresh the cached catalog statistic in the same import transaction so
-- the books list, novel page, and import response cannot drift apart.
create or replace function public.refresh_novel_chapter_count(target_novel_id bigint)
returns integer
...
revoke all on function public.refresh_novel_chapter_count(bigint) from public;
...
begin
  ...
  from jsonb_array_elements(coalesce(import_chapters, '[]'::jsonb))
    with ordinality as c(chapter, ordinal)
  ...
  select public.refresh_novel_chapter_count(target_novel_id) into total_count;
...
-- Repair statistics left stale by earlier imports immediately on deployment.
update public.novels novel
set chapters = (
  select count(distinct chapter.number)
  from public.chapters chapter
