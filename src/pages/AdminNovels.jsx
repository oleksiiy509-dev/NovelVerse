...
function AdminNovels() {
  ...
  const categories = useMemo(() => [...new Set(novels.flatMap((n) => [n.category, ...(n.genres || "").split(",")].map((x) => x?.trim()).filter(Boolean)))], [novels]);
  const filtered = useMemo(() => novels.filter((n) => (!search || [n.title,n.author,n.description,n.tags,n.genres,n.category].some((v)=>norm(v).includes(norm(search)))) && (category === "all" || [n.category,n.genres,n.tags].some((v)=>norm(v).includes(norm(category)))) && (status === "all" || n.status === status)), [novels, search, category, status]);
