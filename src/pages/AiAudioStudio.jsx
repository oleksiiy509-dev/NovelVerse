...
import { applySoundDesignPlan, calculateSoundDesignScore, createSoundDesignPlan, generateSoundDesignDiff, generateSoundDesignReport, revertSoundDesignPlan, saveSoundDesignPlan, SOUND_DESIGN_MODES } from "../lib/soundDesignEngine";
import AssetLibrary from "../components/AssetLibrary";
import { AudiobookPipeline, AUDIOBOOK_STAGE_GRAPH, PIPELINE_STATUSES } from "../lib/audiobookPipeline";
import { createAudiobookPipelineHandlers } from "../lib/audiobookPipelineHandlers";
...
function AiAudioStudio() {
  ...
  const [soundMode, setSoundMode] = useState(SOUND_DESIGN_MODES.ASSISTED);
  const [soundPlan, setSoundPlan] = useState(null);
  const [soundDiff, setSoundDiff] = useState(null);
  const [soundReportOpen, setSoundReportOpen] = useState(false);
  const [pipelineState, setPipelineState] = useState({ status: PIPELINE_STATUSES.WAITING, stages: AUDIOBOOK_STAGE_GRAPH.map((stage) => ({ ...stage, status: PIPELINE_STATUSES.WAITING, progress: 0 })) });
...
useEffect(() => {
  ...
  function analyzeSoundDesign(sceneId = null) { if (!productionPlan) { setStatus("Analyze production before sound design."); return; } const next = createSoundDesignPlan(productionPlan, project, { mode: soundMode, mixerProject, previousPlan: soundPlan }); if (sceneId && soundPlan) next.scenePlans = next.scenePlans.map((scene) => scene.sceneId === sceneId ? scene : soundPlan.scenePlans.find((old) => old.sceneId === scene.sceneId) || scene); setSoundPlan(saveSoundDesignPlan(next)); setSoundDiff(null); setStatus(sceneId ? `Regenerated sound design for ${sceneId}.` : "Sound-design cue plan generated."); }
  function applySoundDesign() { if (!soundPlan || !mixerProject) { setStatus("Build a mix and generate a cue plan first."); return; } const next = applySoundDesignPlan(mixerProject, soundPlan); setSoundDiff(generateSoundDesignDiff(mixerProject, next)); setMixerProject(saveMixerProject(next)); setSoundPlan(saveSoundDesignPlan({ ...soundPlan, status: "applied" })); setStatus("Cue plan applied as reversible mixer overrides."); }
  function revertSoundDesign(options) { if (!mixerProject) return; setMixerProject(saveMixerProject(revertSoundDesignPlan(mixerProject, options))); setStatus(options?.sceneId ? `Reverted sound design for ${options.sceneId}.` : "Sound-design cue plan fully reverted."); }
  function toggleSoundSceneLock(sceneId) { setSoundPlan((current) => current && saveSoundDesignPlan({ ...current, scenePlans: current.scenePlans.map((scene) => scene.sceneId === sceneId ? { ...scene, manuallyLocked: !scene.manuallyLocked } : scene) })); }
  function exportSoundReport() { if (!soundPlan) return; const blob = new Blob([JSON.stringify(generateSoundDesignReport(soundPlan), null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `sound-design-report-${project.chapterId || "chapter"}.json`; link.click(); URL.revokeObjectURL(url); }
  ...
  async function startAudiobookPipeline(action = "start") {
    try {
      if (!selectedChapterId) throw new Error("Select a chapter before starting the pipeline.");
      const chapter = await fetchAudioStudioChapterText(supabase, selectedChapterId);
      if (!pipelineRef.current) pipelineRef.current = new AudiobookPipeline({ id: `${selectedNovelId}:${selectedChapterId}`, handlers: createAudiobookPipelineHandlers({ existingProject: project, existingMixerProject: mixerProject }), onUpdate: setPipelineState });
      if (action === "restart-failed") await pipelineRef.current.restartFailed(chapter, { project });
      else if (action === "restart") await pipelineRef.current.restart(chapter, { project });
      else await pipelineRef.current.start(chapter, { project });
    } catch (error) { setLoadingError(error.message || "Audiobook pipeline failed."); }
  }
  ...
  return <main className="audio-studio"><header className="studio-hero"><div><p className="eyebrow">AI Audio Studio v1</p><h1>Complete audiobook production interface</h1><p>Layer narration, cast voices, ambience, music and sound effects with non-destructive automation.</p></div><div className="studio-actions"><button onClick={undo} disabled={!history.length}>Undo</button><button onClick={redo} disabled={!future.length}>Redo</button><button onClick={exportJson}>Export project JSON</button><button onClick={() => fileRef.current?.click()}>Import project JSON</button><input ref={fileRef} hidden type="file" accept="application/json" onChange={importJson} /></div></header>
    <section className="studio-panel pipeline-panel"><div className="production-heading"><div><p className="eyebrow">AI Audiobook Pipeline v1</p><h2>End-to-end production</h2></div><strong className={`pipeline-status status-${pipelineState.status.toLowerCase()}`}>{pipelineState.status}</strong></div><div className="studio-actions"><button onClick={() => startAudiobookPipeline()}>Start pipeline</button><button onClick={() => pipelineRef.current?.resume()}>Resume</button><button onClick={() => pipelineRef.current?.pause()}>Pause</button><button onClick={() => pipelineRef.current?.cancel()}>Cancel</button><button onClick={() => startAudiobookPipeline("restart-failed")}>Restart failed stage</button><button onClick={() => startAudiobookPipeline("restart")}>Restart whole pipeline</button></div><div className="pipeline-stages">{pipelineState.stages.map((stage) => <article key={stage.id}><div><strong>{stage.label}</strong><span>{stage.status}</span></div><progress max="100" value={stage.progress || 0} /><small>{stage.progress || 0}%{stage.cached ? " · cache reused" : ""}{stage.error ? ` · ${stage.error}` : ""}</small></article>)}</div>{pipelineState.report && <details><summary>Pipeline report</summary><pre>{JSON.stringify(pipelineState.report, null, 2)}</pre></details>}</section>
    ...
    <section className="studio-panel production-engine sound-design-engine"><div className="production-heading"><div><p className="eyebrow">AI Sound Design Engine v1</p><h2>Editable ambience, music &amp; SFX cues</h2></div><label>Sound-design mode<select value={soundMode} onChange={(event) => setSoundMode(event.target.value)}><option value={SOUND_DESIGN_MODES.MANUAL}>Manual</option><option value={SOUND_DESIGN_MODES.ASSISTED}>Assisted</option><option value={SOUND_DESIGN_MODES.FULL_AUTO}>Full Auto</option></select></label></div><div className="studio-actions"><button onClick={() => analyzeSoundDesign()}>Analyze Sound Design</button><button disabled={!productionPlan} onClick={() => analyzeSoundDesign()}>Generate Cue Plan</button><button disabled={!soundPlan || !mixerProject} onClick={applySoundDesign}>Apply Cue Plan</button><button disabled={!soundPlan} onClick={() => setStatus("Cue preview is routed through mixer overrides with speech ducking.")}>Preview Cues</button><button disabled={!mixerProject?.soundDesign} onClick={() => revertSoundDesign()}>Revert Cue Plan</button><button disabled={!soundPlan} onClick={exportSoundReport}>Export Sound Design Report</button></div>{soundPlan && <><div className="production-summary"><strong>Status: {soundPlan.status}</strong><strong>Overall score: {calculateSoundDesignScore(soundPlan).overallScore}</strong><span>Confidence: {Math.round(soundPlan.confidence.value * 100)}%</span><span>{soundPlan.warnings.length} warnings</span></div><div className="production-scenes">{soundPlan.scenePlans.map((scene) => <article key={scene.sceneId}><div><strong>{scene.sceneId}</strong><span className={`confidence confidence-${scene.confidence.level}`}>{Math.round(scene.confidence.value * 100)}%</span>{scene.manuallyLocked && <span className="lock-badge">🔒 Locked</span>}</div><h3>{scene.environment}</h3><p>{scene.location} · {scene.weather} · {scene.timeOfDay}</p><p title={scene.confidence.reason}>🌿 {scene.ambienceLayers.length} ambience · 🎵 {scene.musicCues.length} music · ✦ {scene.sfxCues.length} SFX · ◌ {scene.silenceRegions.length} silence</p><div className="cue-list">{[...scene.ambienceLayers, ...scene.musicCues, ...scene.sfxCues].map((cue, index) => <span className={cue.missingAsset ? "missing-warning" : ""} key={cue.cueId || `${cue.category}-${index}`}>{cue.label || cue.mood} {cue.missingAsset ? "⚠ missing asset" : "✓ asset"}</span>)}</div><div><button onClick={() => toggleSoundSceneLock(scene.sceneId)}>{scene.manuallyLocked ? "Unlock Scene" : "Lock Scene"}</button><button onClick={() => analyzeSoundDesign(scene.sceneId)}>Regenerate Scene</button><button disabled={!mixerProject?.soundDesign} onClick={() => revertSoundDesign({ sceneId: scene.sceneId })}>Revert scene</button></div></article>)}</div><div className="production-lower"><article><h3>Cue diff</h3>{soundDiff ? <><p>{soundDiff.addedCues.length} added · {soundDiff.removedCues.length} removed · {soundDiff.modifiedCues.length} modified</p>{soundDiff.changes.slice(0, 10).map((change) => <p key={change.clipId}><strong>{change.cueId}</strong> — {change.changeType}; {change.timingChange ? "timing changed" : "timing preserved"}; {change.assetChange ? "asset changed" : "asset preserved"}. {change.reason}</p>)}<p>{soundDiff.preservedManualEdits.length} manual edits preserved.</p></> : <p>Apply the cue plan to see added, removed, modified, asset, timing and confidence changes.</p>}</article><article><button onClick={() => setSoundReportOpen((open) => !open)}>Sound-design report panel</button>{soundReportOpen && <pre>{JSON.stringify(generateSoundDesignReport(soundPlan), null, 2)}</pre>}</article></div></>}</section>
    ...
    <AssetLibrary soundPlan={soundPlan} onSoundPlanChange={(next) => setSoundPlan(saveSoundDesignPlan(next))} />
