...
function BetaDashboard() {
  const repository = useRef(new BetaRepository());
  const queue = useRef(null);
  const [state, setState] = useState(repository.current.snapshot());
  const [diagnostics, setDiagnostics] = useState(null);
  const [tab, setTab] = useState("pipeline");
  const refresh = () => setState(repository.current.snapshot());
  if (!queue.current) queue.current = new BackgroundQueue({ repository: repository.current, onUpdate: refresh });
  useEffect(() => { if (repository.current.state.settings.pipeline.autoResume) queue.current.resumeAll(); }, []);
  ...
  const start = async () => { let project = state.projects[0]; if (!project) project = repository.current.upsertProject({ name: "Untitled Beta Project", chapters: ["Chapter 1"] }); const job = queue.current.add({ projectId: project.id, chapterIds: project.chapters }); refresh(); await queue.current.run(job.id, { project }); };
  ...
  return <main className="beta-shell">
    ...
    {!state.firstLaunchComplete && <section className="wizard"><b>Welcome to NovelVerse Beta</b><span>Connect the local worker, choose defaults, then launch your first production.</span><button onClick={() => { repository.current.state.firstLaunchComplete = true; repository.current.save(); refresh(); }}>Finish setup</button></section>}
    ...
    {tab === "pipeline" && <><section className="metric-grid"><article><small>QUEUE</small><strong>{metrics.queueSize}</strong><span>active jobs</span></article><article><small>PROGRESS</small><strong>{active?.progress || 0}%</strong><span>{active?.status || "ready"}</span></article><article><small>ELAPSED</small><strong>{fmt(active?.elapsedMs)}</strong><span>remaining {fmt(active?.remainingMs)}</span></article><article><small>CACHE</small><strong>{metrics.cacheHits}/{metrics.cacheMisses}</strong><span>hits / misses</span></article></section>
      ...
      <div className="actions"><button onClick={() => active && queue.current.pause(active.id)}>Stop</button><button onClick={() => active && queue.current.run(active.id)}>Resume</button><button onClick={() => active && queue.current.stop(active.id)}>Cancel</button></div></>}
    {tab === "projects" && <section className="panel"><h2>Project Manager</h2>{state.projects.map((project) => <div className="row" key={project.id}><b>{project.name}</b><span>{project.updatedAt}</span><button>Open</button><button onClick={() => { repository.current.duplicate(project.id); refresh(); }}>Duplicate</button><button onClick={() => { repository.current.archive(project.id); refresh(); }}>Archive</button><button onClick={() => { repository.current.delete(project.id); refresh(); }}>Delete</button></div>)}<h3>Archive</h3>{state.archivedProjects.map((project) => <div className="row" key={project.id}><b>{project.name}</b><button onClick={() => { repository.current.restore(project.id); refresh(); }}>Restore</button></div>)}</section>}
