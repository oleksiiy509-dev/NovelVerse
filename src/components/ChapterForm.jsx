...
function ChapterForm({ initialChapter, chapterId }) {
  ...
  async function save(e){ e.preventDefault(); setSaving(true); const payload={ novel_id:Number(form.novel_id), title:form.title, number:Number(form.number), content: htmlToText(form.content) }; const result = chapterId ? await supabase.from("chapters").update(payload).eq("id", chapterId) : await supabase.from("chapters").insert(payload); setSaving(false); if(result.error) alert(result.error.message); else { localStorage.removeItem(draftKey); navigate("/admin/chapters"); } }
