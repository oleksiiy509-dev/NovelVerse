...
export default function BookManagement({ onBooksChange, bookId, createNew = false, initialTab = "Metadata" }) {
  ...
  return <section className="book-cms">
    ...
    <div className="cms-layout"><aside className="cms-books">{shown.map((book) => <button key={book.id} className={book.id === selectedId ? "selected" : ""} onClick={() => { setChapterId(""); navigate(`/admin/books/${book.id}`); }}><span className="cms-cover" style={book.coverUrl ? { backgroundImage: `url(${book.coverUrl})` } : {}}>{!book.coverUrl && book.title.slice(0, 2).toUpperCase()}</span><span><b>{book.title}</b><small>{book.author || "Author not set"}</small><em>{book.status}</em></span></button>)}</aside>
    {selected ? <main className="cms-editor"><header className="cms-editor-head"><div><span className="cms-eyebrow">CONTENT MANAGEMENT / {selected.status.toUpperCase()}</span><h2>{selected.title}</h2><p>{selected.chapters.length} chapters · {selected.language} · {selected.ageRating}</p></div><div><button className="cms-danger" onClick={remove}>Delete</button><button onClick={persist} disabled={saving}>{saving ? "Saving…" : "Save changes"}</button></div></header>
      ...
      {tab === "Audio" && <div className="audio-manager"><div className="audio-callout"><div><b>Generate the complete audiobook</b><p>Creates playable narration for every chapter that does not have audio.</p></div><button onClick={generateWholeBook} disabled={generating.length > 0 || !selected.chapters.length}>{generating.length ? "Generating…" : "Generate entire book"}</button></div>{selected.chapters.map((item, index) => <article key={item.id}><span>▶</span><div><b>{index + 1}. {item.title}</b><small>{item.audioStatus === "Ready" ? "Audio ready for preview" : item.audioStatus === "Generating" ? "Generating real narration…" : "No generated audio"}</small></div><em className={item.audioStatus.toLowerCase()}>{item.audioStatus}</em>{item.audioUrl && <audio controls preload="metadata" src={item.audioUrl}>Your browser cannot play this audio.</audio>}<button onClick={() => generateAudio(item)} disabled={generating.includes(item.id)}>{generating.includes(item.id) ? "Generating…" : item.audioStatus === "Ready" ? "Regenerate" : "Generate chapter"}</button>{item.audioStatus === "Ready" && <button className="cms-danger" onClick={() => update({ chapters: selected.chapters.map((entry) => entry.id === item.id ? { ...entry, audioStatus: "Missing", audioUrl: "" } : entry) })}>Delete</button>}</article>)}</div>}
