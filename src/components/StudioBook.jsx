...
export default function StudioBook({ novelId, mode = "chapters" }) {
  ...
  return <section className="studio-book"><header><div><h2>{novel.title}</h2><p>{novel.author || "Unknown author"} · {chapters.length} chapters</p></div><div><button onClick={() => navigate(`${base}/add-chapter`)}>Add Chapter</button><button onClick={() => navigate(`${base}/import-chapters`)}>Import Chapters</button></div></header><div className="studio-book__chapters">{chapters.map((chapter) => <article key={chapter.id} onClick={() => navigate(`/reader/${chapter.id}`)}><strong>Chapter {chapter.number}</strong><span>{chapter.title}</span></article>)}{!chapters.length && <p>No chapters yet.</p>}</div></section>;
}
