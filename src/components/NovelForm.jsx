...
const emptyNovel = { title:"", author:"", image:"", description:"", chapters:"", rating:"", content:"", genres:"", status:"Ongoing", views:0, bookmarks:0 };
...
function NovelForm({ initialNovel, novelId }) {
  ...
  const [form, setForm] = useState(() => { const saved = JSON.parse(localStorage.getItem(draftKey) || "{}"); const initialData = toNovelFields(initialNovel); const savedData = toNovelFields(saved); const genres = [initialData.genres, saved.category, savedData.genres].filter(Boolean).join(", "); return { ...emptyNovel, ...initialData, ...savedData, genres }; });
  ...
  function generateDescription(){ setForm((f)=>({ ...f, ...buildNovelMetadata(f) })); }
  ...
  return <form className="admin-form" onSubmit={save}>
    <div className="admin-form-grid">
      ...
      <label className="admin-full">Контент<textarea rows={8} value={form.content || ""} onChange={(e)=>field("content", e.target.value)} placeholder="Текст новели або чернетка контенту" /></label>
    ...
    {preview && <section className="admin-preview"><h2>{form.title || "Untitled"}</h2><p className="admin-muted">{form.author} • {form.genres} • {form.status}</p><p>{form.description || metadata.description}</p><div className="admin-chip-list">{(form.genres || "").split(",").filter(Boolean).map((genre)=><span className="admin-chip" key={genre}>{genre.trim()}</span>)}</div></section>}
    <div className="admin-actions"><button type="button" className="admin-secondary" onClick={generateDescription}>✨ Згенерувати опис</button><button type="button" className="admin-secondary" onClick={()=>setPreview((v)=>!v)}>👁️ Preview</button><button disabled={saving}>{saving ? "Збереження..." : "💾 Зберегти"}</button><button type="button" className="admin-secondary" onClick={()=>navigate("/admin/novels")}>Скасувати</button></div>
