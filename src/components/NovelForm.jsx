...
const emptyNovel = { title:"", author:"", slug:"", description:"", meta_title:"", meta_description:"", keywords:"", rating:"", chapters:"", image:"", genres:"", category:"", tags:"", status:"Ongoing", views:0, bookmarks:0 };

function NovelForm({ initialNovel, novelId }) {
  const navigate = useNavigate();
  const draftKey = `novelverse:novel-draft:${novelId || "new"}`;
  const [form, setForm] = useState(() => ({ ...emptyNovel, ...(initialNovel || {}), ...(JSON.parse(localStorage.getItem(draftKey) || "{}")) }));
  ...
  return <form className="admin-form" onSubmit={save}>
    <div className="admin-form-grid">
      ...
      <label>Категорія<input value={form.category || ""} onChange={(e)=>field("category", e.target.value)} placeholder="Fantasy" /></label>
      <label>Жанри<input value={form.genres || ""} onChange={(e)=>field("genres", e.target.value)} placeholder="Action, Adventure" /></label>
    ...
    {preview && <section className="admin-preview"><h2>{form.title || "Untitled"}</h2><p className="admin-muted">{form.author} • {form.category || form.genres} • {form.status}</p><p>{form.description || metadata.description}</p><div className="admin-chip-list">{(form.tags || metadata.keywords).split(",").filter(Boolean).map((t)=><span className="admin-chip" key={t}>{t.trim()}</span>)}</div></section>}
